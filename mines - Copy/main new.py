...
# Import crash predictor modules
try:
    ...
    logger.warning(f"Crash predictor modules not available: {e}")
...
# Initialize crash predictor if available
if CRASH_PREDICTOR_AVAILABLE:
    crash_bot = StakeCrashBot()
    crash_predictors = {}  # Store predictors per user
else:
    crash_bot = None
    crash_predictors = {}
...
# Function to fetch game data from Stake.ac API
def fetch_stake_game_data(access_token):
    api_url = "https://stake.ac/_api/casino/active-bet/mines"
    ...
    for attempt in range(max_retries):
        try:
            logger.info(f"Sending POST request to stake.ac Mines API... (Attempt {attempt+1}/{max_retries})")
...
# Add crash predictor endpoints
@app.route('/crash_predict', methods=['POST', 'OPTIONS'])
def crash_predict_endpoint():
    """Crash predictor endpoint"""
    if request.method == 'OPTIONS':
        response = jsonify({'status': 'ok'})
        response.headers.add('Access-Control-Allow-Origin', '*')
    ...
    if not CRASH_PREDICTOR_AVAILABLE:
        return jsonify({'status': 'error', 'error': 'Crash predictor not available'}), 503
    ...
    try:
        ...
        if user_id not in crash_predictors:
            crash_predictors[user_id] = AdvancedCrashPredictor()
        
        predictor = crash_predictors[user_id]
...
def crash_history_endpoint():
    """Fetch crash history endpoint"""
    if not CRASH_PREDICTOR_AVAILABLE:
        return jsonify({'status': 'error', 'error': 'Crash predictor not available'}), 503
    ...
    try:
        ...
        logger.error(f"Crash history error: {e}")
        return jsonify({'status': 'error', 'error': str(e)}), 500

if __name__ == '__main__':
    # Crash predictor is now integrated directly
    logger.info("Starting Flask app with integrated crash predictor...")
