...
app = Flask(__name__)
CORS(app, resources={r"/*": {"origins": "*"}}, allow_headers=["Content-Type", "X-User-Identifier", "X-Device-ID", "Origin", "Accept"])  # ✅ Updated CORS fix
...
def login():
    if request.method == 'OPTIONS':
        ...
        response.headers.add('Access-Control-Allow-Headers', 'Content-Type,X-User-Identifier,X-Device-ID,Origin,Accept')
    ...
    try:
        ...
        # Check for device ID in either X-Device-ID or X-User-Identifier header
        device_id = request.headers.get('X-Device-ID') or request.headers.get('X-User-Identifier')
        if not device_id:
            return jsonify({
                "status": "error",
