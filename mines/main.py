        ...
        if game_type == 'mines':
            ...
            if is_active and bet_id:
                ...
                if last_bet != bet_id:
                    ...
                    logger.info(f"🎲 New bet detected for {token[:10]}... Bet ID: {bet_id}")
                    
                    # Generate mines prediction using random algorithm
                    import random
                    ...
                    logger.info(f"💎 Mines prediction generated for {token[:10]}... (Gems: {len(gem_positions)}, Bombs: {len(bomb_positions)})")
                else:
...
if __name__ == '__main__':
    ...
    logger.info("")
    app.run(host='127.0.0.1', port=2163, debug=True)
